...
export async function analyzeDebt(debtInput: AnalyzeDebtInput): Promise<AnalyzeDebtOutput> {
    try {
      const result = await analyzeDebtFlow(debtInput);
      return result;
    } catch(e: any) {
        console.error("Error analyzing debt:", e);
        throw new Error("La IA no pudo analizar la deuda. Por favor, verifica los datos e intenta de nuevo.");
    }
}
